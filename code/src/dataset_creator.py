    ...
    for img_path in image_path_list[1:]:
        image = np.concatenate(
            (
                image, 
                np.expand_dims(
                    cv2.resize(cv2.imread(img_path, 1), (48, 48)),
                    axis=0
                )
            ), 
            axis=0
...
def create_dataset(images, labels):
    print('create_dataset....',images.shape)
    images = np.reshape(images, (-1, 48*48*3))
    print('create_dataset....processed images' , images.shape)
    logger.debug('images-shape: {}, length-labels: {}'.format(
        images.shape, len(labels)
    ))
    
    train, test = train_test_splitter(images, labels)
    print('create_dataset....processed images-train' , train[0].shape)
    print('create_dataset....processed images-test' , test[0].shape)
    train_dict = {
        'data': train[0],
        'labels': train[1]
    }
